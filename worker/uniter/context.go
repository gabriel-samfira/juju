...
type HookContext struct {
	...
	// rebootPrio tells us when the hook wants to reboot. If rebootPrio is jujuc.RebootNow
...
func (ctx *HookContext) RequestReboot(prio jujuc.RebootPriority) error {
	if ctx.pid == 0 {
		// not really an error. This happens when run via juju-run. No hook
		// to record the pid.
		logger.Warningf("Reboot requested from outside a hook")
	}
	ctx.rebootPrio = prio
	if ctx.rebootPrio == jujuc.RebootNow && ctx.pid != 0 {
...
func (ctx *HookContext) finalizeContext(process string, err error) error {
	...
	for id, rctx := range ctx.relations {
		// immediate reboot was requested. Error generated by killing
		// the hook forcefully is ignored here. We need to write the relation
		// settings made so far
		if writeChanges || ctx.rebootPrio != jujuc.RebootSkip {
...
func (ctx *HookContext) RunCommands(commands, charmDir, toolsDir, socketPath string) (*utilexec.ExecResponse, error) {
	...
	result, err := utilexec.RunCommands(
		utilexec.RunParams{
			...
			WorkingDir:  charmDir,
			Environment: env})
	if ctx.rebootPrio != jujuc.RebootSkip {
		err = ctx.unit.RequestReboot()
	}
