...
func (info *machineInfo) cloudinitRunCmd(series string) (string, error) {
	...
	render, err := cloudinit.NewRenderer(series)
	if err != nil {
		return "", err
	}
	...
	fileName := render.PathJoin(dataDir, "MAASmachine.txt")
	script := render.MkDir(dataDir)
	contents := utils.ShQuote(string(yaml))
	script = append(script, render.WriteFile(fileName, contents, 0755)...)
